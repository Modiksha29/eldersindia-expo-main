...
import { updateEvents, getUnapprovedEvents, destroyEvents } from 'apis/admin/events'
...
export default function Page() {
  ...
  const fetchEvents = (page = 1, sort, search) => getUnapprovedEvents(page, sort, search)
  ...
  const {
    ...
    // enabled: false,
    queryKey: ['unapprovedevents', page, sort, search],
  ...
  useMutation({
    ...
    onMutate: async (newEvent) => {
      ...
      queryClient.setQueryData(['id, is_approved'], (old) => [...old, newEvent])
  ...
  const eventsApproveMutation = useMutation({
    mutationFn: (data) => updateEvents(data),
    onSuccess(data) {
  ...
  const eventsRejectMutation = useMutation({
    mutationFn: (data) => destroyEvents(data),
    onSuccess(data) {
      console.log(data?.data?.data);
      showToast();
    },
      ...
      <MotiView style={{ flex: 1, padding: 4 }}>
        ...
        >
          ...
          <MotiView style={{ flexDirection: 'row', marginTop: 8, justifyContent: 'space-between' }} >
            ...
            <Button
              onPress={() => eventsApproveMutation.mutate({ id: item.id, is_approved: 1 })}
            ...
            <Button
              onPress={() => eventsRejectMutation.mutate(item.id)}
            ...
            <Button
              onPress={() => push(`/admin/events/${item.id}/${item.name}`)}
  ...
  if (isError) {
    return <Layout level='1' style={{ flex: 1 , justifyContent:'center', alignItems: 'center',}}>
      ...
      <Text category='s1' status='warning' style={{ textAlign: 'center', marginVertical: 8 }}>Failed to get approved events details.</Text>
  ...
  <Layout style={{flex:1}}>
    {eventsdata.length > 0 ? (
      ...
      <Layout style={{ marginTop:"50%",justifyContent: 'center', alignItems: 'center', }}>
        ...
        <Text category='s1' status='warning' style={{ textAlign: 'center', marginVertical: 8 }}>No events for approval.</Text>
        <Button status='primary' appearance='outline' onPress={() => refetch()}
          style={{ width: "50%" }} accessoryLeft={
            <RetryIcon color={theme['color-primary-default']} />}>Retry
        </Button>
